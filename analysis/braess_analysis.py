...
def write_ratios_csv(ratios, baselines, faults):
...
def write_latex_table(ratios, baselines, faults):
    """
    Table: Braess ratio (fault_throughput / baseline_throughput) per solver × density.
    One subtable per scenario. Dagger marks Braess benefit (ratio > 1, CI > 1).
    """
    ...
    lines.append("% Values: mean ratio [95\\% CI]; \\dag = ratio > 1 (Braess benefit)")
    lines.append("")
    ...
    for scenario in SCENARIO_ORDER:
        ...
        for density in DENSITY_ORDER:
            ...
            for solver in SOLVER_ORDER:
                ...
                sig = p < 0.05 and not math.isnan(p)

                if r_mean > 1.0 and r_lo > 1.0:
                    # Both mean and CI lower bound above 1 → confirmed Braess benefit
        ...
        lines.append(f"      \\footnotesize $\\dag$ ratio $>$ 1 (Braess benefit, CI lower bound $>$ 1); "
                     f"$^*$ significant ($p < 0.05$, Mann-Whitney U).}}")
...
def print_summary(ratios, baselines, faults):
    print("\n" + "="*70)
    print("BRAESS PARADOX ANALYSIS — MAFIS Paper")
    print("="*70)
    print(f"{'Solver':<20} {'Density':>7} {'Scenario':<25} {'Ratio':>8} {'95% CI':>18} {'p-value':>10}")
    print("-"*70)
    ...
    for solver in SOLVER_ORDER:
        for density in DENSITY_ORDER:
            for scenario in SCENARIO_ORDER:
                ...
                lo, hi = ci95(rs)
                _, p = mann_whitney_u(ft, bl)
                braess = " ←BRAESS" if r > 1.0 and lo > 1.0 else ""
                sig = "*" if p < 0.05 else " "
                print(f"{SOLVER_LABEL[solver]:<20} {density:>7} {scenario:<25} {r:>8.3f} [{lo:.3f}, {hi:.3f}] {format_p(p):>10}{sig}{braess}")

    print("\n--- Braess threshold per solver (lowest density where ratio > 1, CI > 1) ---")
    for solver in SOLVER_ORDER:
        threshold = None
        for density in DENSITY_ORDER:
            # Average over all permanent-distributed scenarios (burst/wear)
            permanent_dist = ["burst_20pct", "burst_50pct", "wear_medium", "wear_high"]
            rs_all = []
            for sc in permanent_dist:
                rs_all.extend(ratios.get((solver, density, sc), []))
            if rs_all:
                r = mean(rs_all)
                lo, _ = ci95(rs_all)
                if r > 1.0 and lo > 1.0 and threshold is None:
                    threshold = density
        if threshold:
            print(f"  {SOLVER_LABEL[solver]:<20} → Braess threshold at n={threshold}")
        else:
...
def main():
    ...
    print("\nWriting outputs...")
    write_ratios_csv(ratios, baselines, faults)
    write_significance_csv(baselines, faults)
    write_latex_table(ratios, baselines, faults)
    write_degradation_svg(ratios)

    print_summary(ratios, baselines, faults)
